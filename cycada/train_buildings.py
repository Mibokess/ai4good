...
net_type = "UNet_splits_new_buildings_jitter"
os.makedirs(f"/cluster/scratch/mboss/logs/{net_type}", exist_ok=True)
epochs = 250
...
project_name = "unet"
log_path = f"/cluster/scratch/mboss/logs/{net_type}/"
wandb_logger = pl_loggers.WandbLogger(project=project_name, id=net_type, save_dir=log_path)
lr_logger = LearningRateMonitor(logging_interval = 'epoch')
checkpoint_callback = ModelCheckpoint(f'/cluster/scratch/mboss/logs/lightning_logs/{net_type}/', monitor = "validation_f1", save_top_k = 3, save_last = True, mode='max')
